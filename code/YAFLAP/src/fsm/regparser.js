import * as DotConverter from './dot-converter'
import { Lexer, EMPTYTOKEN, TOKEN_TYPE } from './lexer'
  ...
  for (var id in nfaGraph) {
    for (var j = 0; j < nfaGraph[id].length; ++j) {
      ...
      if (!alphabetTable.hasOwnProperty(label) &&
          label != TOKEN_TYPE.EMPTY) { alphabetTable[label] = 1 }
    }
  }
...
FSM.prototype.match = function (text) {
  ...
  for (var i = 0; i < text.length; ++i) {
    if (!this.transitions[currentState]) { return false }
    var notFound = true
...
RegParser.prototype._traversalFSM = function () {
  ...
  while (queue.length) {
    ...
    for (var i = 0; i < (state.nextStates).length; ++i) {
      ...
      vis[nextId] = 1
      if (nextState.isAccept) { fsm.acceptStates.push(nextId.toString()) }
      queue.push(state.nextStates[i][1])
    }
  }
  return fsm
...
RegParser.prototype._match = function (type) {
  if (this.lookHead.type == type) { this._consume() } else { throw new Error('Cannot match type: ' + this.lookHead.text) }
}
